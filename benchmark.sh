#!/bin/bash
sbt -batch -java-home /usr/lib/jvm/graalvm-jdk-22 clean 'jsoniter-scala-benchmarkJVM/jmh:run -jvmArgsAppend "-Djmh.executor=FJP" -p size=128 -prof gc -rf json -rff graalvm-jdk-22.json .*' 2>&1 | tee graalvm-jdk-22.txt
...
sbt -batch -java-home /usr/lib/jvm/graalvm-community-jdk-24 clean 'jsoniter-scala-benchmarkJVM/jmh:run -jvmArgsAppend "-Djmh.executor=FJP" -p size=128 -prof gc -rf json -rff graalvm-community-jdk-24.json .*' 2>&1 | tee graalvm-community-jdk-24.txt
...
sbt -batch -java-home /usr/lib/jvm/jdk-24 clean 'jsoniter-scala-benchmarkJVM/jmh:run -jvmArgsAppend "-Djmh.executor=FJP" -p size=128 -prof gc -rf json -rff jdk-24.json .*' 2>&1 | tee jdk-24.txt
