#!/bin/bash
sbt -batch -java-home /usr/lib/jvm/graalvm-ee-java19 -Dmacro.settings=print-codecs clean 'jsoniter-scala-benchmarkJVM/jmh:run -p size=128 -prof gc -rf json -rff graalvm-ee-java19.json .*' 2>&1 | tee graalvm-ee-java19.txt
...
sbt -batch -java-home /usr/lib/jvm/graalvm-ce-java19 -Dmacro.settings=print-codecs clean 'jsoniter-scala-benchmarkJVM/jmh:run -p size=128 -prof gc -rf json -rff graalvm-ce-java19.json .*' 2>&1 | tee graalvm-ce-java19.txt
...
sbt -batch -java-home /usr/lib/jvm/openjdk-20 -Dmacro.settings=print-codecs clean 'jsoniter-scala-benchmarkJVM/jmh:run -p size=128 -prof gc -rf json -rff openjdk-20.json .*' 2>&1 | tee openjdk-20.txt
