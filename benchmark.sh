...
sbt -java-home /usr/lib/jvm/openjdk-11 -no-colors -Dsbt.supershell=false -Dmacro.settings=print-codecs clean 'jsoniter-scala-benchmark/jmh:run -jvm /usr/lib/jvm/openjdk-12/bin/java -p size=128 -prof gc -rf json -rff openjdk12.json .*' 2>&1 | tee openjdk12.txt
...
sbt -java-home /usr/lib/jvm/java-8-openjdk-amd64 -no-colors -Dsbt.supershell=false -Dmacro.settings=print-codecs clean 'jsoniter-scala-benchmark/jmh:run -p size=128 -prof gc -rf json -rff openjdk8.json .*' 2>&1 | tee openjdk8.txt
