#!/bin/bash
sbt -java-home /usr/lib/jvm/graalvm-ee-1.0.0 -no-colors clean 'jsoniter-scala-benchmark/jmh:run -p size=128 -prof gc -rf json -rff graalvmee1.json .*' 2>&1 | tee graalvmee1.txt
sbt -java-home /usr/lib/jvm/graalvm-ce-1.0.0 -no-colors clean 'jsoniter-scala-benchmark/jmh:run -p size=128 -prof gc -rf json -rff graalvmce1.json .*' 2>&1 | tee graalvmce1.txt
