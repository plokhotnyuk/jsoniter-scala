...
class ArrayOfBooleansReading extends ArrayOfBooleansBenchmark {
  ...
  @Benchmark
  def circeJawn(): Array[Boolean] = io.circe.jawn.decodeByteArray[Array[Boolean]](jsonBytes).fold(throw _, identity)

  @Benchmark
  def circeJsoniter(): Array[Boolean] = {
    import com.github.plokhotnyuk.jsoniter_scala.benchmark.CirceJsoniterCodecs._

    Decoder[Array[Boolean]].decodeJson(readFromArray[io.circe.Json](jsonBytes)).fold(throw _, identity)
  }
  ...
  def playJsonJsoniter(): Array[Boolean] = {
    ...
    readFromArray[play.api.libs.json.JsValue](jsonBytes).as[Array[Boolean]]
  }
