...
class ArrayOfMonthDaysReading extends ArrayOfMonthDaysBenchmark {
  ...
  @Benchmark
  def circeJawn(): Array[MonthDay] = io.circe.jawn.decodeByteArray[Array[MonthDay]](jsonBytes).fold(throw _, identity)
  ...
  def circeJsoniter(): Array[MonthDay] = {
    ...
    Decoder[Array[MonthDay]].decodeJson(readFromArray[io.circe.Json](jsonBytes)).fold(throw _, identity)
  }
  ...
  def playJsonJsoniter(): Array[MonthDay] = {
    ...
    readFromArray[play.api.libs.json.JsValue](jsonBytes).as[Array[MonthDay]]
  }
