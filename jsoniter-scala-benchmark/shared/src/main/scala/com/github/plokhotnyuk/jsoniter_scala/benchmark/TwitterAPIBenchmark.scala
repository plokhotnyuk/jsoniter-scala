...
abstract class TwitterAPIBenchmark extends CommonParams {
  var jsonBytes: Array[Byte] = bytes(Utils.getResourceAsStream("twitter_api_response.json"))
  var compactJsonBytes: Array[Byte] = bytes(Utils.getResourceAsStream("twitter_api_compact_response.json"))
