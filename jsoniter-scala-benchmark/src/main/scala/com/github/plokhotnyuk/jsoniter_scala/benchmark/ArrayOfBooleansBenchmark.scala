...
class ArrayOfBooleansBenchmark extends CommonParams {
  ...
  @Setup
  def setup(): Unit = {
    obj = (1 to size).map(i => ((i * 1498724053) & 0x1) == 0).toArray
  ...
  @Benchmark
  def readDslJsonScala(): Array[Boolean] = dslJsonDecode[Array[Boolean]](jsonBytes)
  ...
  @Benchmark
  def readSprayJson(): Array[Boolean] = JsonParser(jsonBytes).convertTo[Array[Boolean]]
