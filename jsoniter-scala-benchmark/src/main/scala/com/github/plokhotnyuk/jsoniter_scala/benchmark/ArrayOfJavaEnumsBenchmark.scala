...
class ArrayOfJavaEnumsBenchmark extends CommonParams {
  ...
  @Benchmark
  def readBorerJson(): Array[Suit] = io.bullet.borer.Json.decode(jsonBytes).to[Array[Suit]].value

  @Benchmark
  def readCirce(): Array[Suit] = decode[Array[Suit]](new String(jsonBytes, UTF_8)).fold(throw _, identity)

  @Benchmark
  def readDslJsonScala(): Array[Suit] = dslJsonDecode[Array[Suit]](jsonBytes)
  ...
  @Benchmark
  def readPlayJson(): Array[Suit] = Json.parse(jsonBytes).as[Array[Suit]]

  @Benchmark
  def readSprayJson(): Array[Suit] = JsonParser(jsonBytes).convertTo[Array[Suit]]
