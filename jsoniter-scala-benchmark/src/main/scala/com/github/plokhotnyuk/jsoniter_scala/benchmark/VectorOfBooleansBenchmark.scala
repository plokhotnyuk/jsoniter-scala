...
class VectorOfBooleansBenchmark extends CommonParams {
  ...
  @Benchmark
  def readDslJsonScala(): Vector[Boolean] = dslJsonDecode[Vector[Boolean]](jsonBytes)
  ...
  @Benchmark
  def readSprayJson(): Vector[Boolean] = JsonParser(jsonBytes).convertTo[Vector[Boolean]]
