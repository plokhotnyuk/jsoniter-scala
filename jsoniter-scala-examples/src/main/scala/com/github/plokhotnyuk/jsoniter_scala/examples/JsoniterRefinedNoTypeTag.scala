...
trait JsoniterRefinedNoTypeTag {
    ...
    new JsonCodec[F[Int, P]] {
      ...
      override def decodeKey(in: JsonReader): F[Int, P] = {
        ...
        refType refine unrefined match {
          case Left(because) =>
            in decodeError s"Cannot refined unrefined Int '$unrefined' because: $because"
          case Right(refined) =>
            refined
        }
      }
  ...
  implicit def stringCodec[F[_, _], P](
    ...
    new JsonCodec[F[String, P]] {
      ...
      override def decodeKey(in: JsonReader): F[String, P] = {
        ...
        refType refine unrefined match {
          case Left(because) =>
            in decodeError s"Cannot refined unrefined String '$unrefined' because: $because"
          case Right(refined) =>
            refined
        }
      }
      ...
      override def decodeValue(in: JsonReader,
        ...
        if (in isNextToken 'n') {
          ...
          refType refine unrefined match {
            case Left(because) =>
              in decodeError s"CCannot refined unrefined String '$unrefined' because: $because"
            case Right(refined) =>
              refined
          }
        }
